/* bar.js — Le Bar de Croissantage
   Deux modules :
   - Drunk : moteur d'ivresse global (0-100). Applique des classes
     visuelles sur <body> (flou, teinte, tangage) + pilote un filtre
     SVG de distorsion "vagues" (feTurbulence/feDisplacementMap) pour
     un vrai effet d'écran qui ondule, comme si le sol tanguait.
     L'effet suit le joueur partout (roulette, blackjack...) et
     redescend tout seul avec le temps.
   - Bar : la scène (bar en bois, étagères de bouteilles, barman,
     comptoir) + le menu des boissons, achetées via Wallet.
*/
const Drunk = (() => {
  const MAX = 100;
  ...
  const TICK_MS = 4000;
  let level = 0;
  let currentTierClass = null; // pour n'appliquer/retirer les classes que si le palier change vraiment
...
const Bar = (() => {
  ...
  const REMEDIES = [
    { id: 'water', emoji: '🥤', name: 'Eau fraîche', price: 10, dose: -20, tagline: 'Un petit remède rapide.' },
    { id: 'coffee', emoji: '☕', name: 'Café serré', price: 15, dose: -26, tagline: 'Ça réveille direct.' },
  ];
